...
filters.getEligibleDob = (data) => {

    const outputDateFormat = 'd MMMM yyyy'
    const carerDateOfBirth = `${data['carerDateOfBirth-day']} ${data['carerDateOfBirth-month']} ${data['carerDateOfBirth-year']}`
    return DateTime.fromFormat(carerDateOfBirth, 'd M yyyy').plus({ years: 15, months: 9 }).toFormat(outputDateFormat)
  }
  ...
  filters.claimDateMinusThreeYears = (data) => {
    const outputDateFormat = 'd MMMM yyyy'
    const claimStartDate = `${data['carerClaimStart-day']} ${data['carerClaimStart-month']} ${data['carerClaimStart-year']}`
    return DateTime.fromFormat(claimStartDate, 'd M yyyy').minus({ years:3 }).toFormat(outputDateFormat)
  }
  ...
  filters.calculateRecommendedDateOfClaim = (data) => {
    ...
    const threeMonthsAgo = DateTime.now().minus({ months: 3 })
     ...
     // Claim date minus 3 years
 //  const claimDateMinusThreeYears = carerClaimStart.fromFormat().minus({ years: 3 })
  ...
  filters.dateThreeMonthsAhead = () => {
    const date = DateTime.now().plus({ months: 3 })
    return date.toFormat('d MMMM yyyy') // 10 January 2022
  }
  ...
  filters.getProvidingCare = (data) => {
    const outputDateFormat = 'd MMMM yyyy'
    const providingCare = `${data['providingCare-day']} ${data['providingCare-month']} ${data['providingCare-year']}`
    return DateTime.fromFormat(providingCare, 'd M yyyy').toFormat(outputDateFormat)
  }
  ...
  filters.getDpDateOfBirth = (data) => {
    const outputDateFormat = 'd MMMM yyyy'
    const dateOfBirth = `${data['careeDateOfBirth-day']} ${data['careeDateOfBirth-month']} ${data['careeDateOfBirth-year']}`
    return DateTime.fromFormat(dateOfBirth, 'd M yyyy').toFormat(outputDateFormat)
  }

  filters.getCarerName = (data) => {
  ...
  filters.getQualifyingBenefit = (data) => {
    const benefits = {
      ...
      EU: 'disability benefit'
    }
    return benefits[data['elig--qb']]
  }

  filters.getCarerDateOfBirth = (data) => {
    const outputDateFormat = 'd MMMM yyyy'
    const claimStartDate = `${data['carerDateOfBirth-day']} ${data['carerDateOfBirth-month']} ${data['carerDateOfBirth-year']}`
